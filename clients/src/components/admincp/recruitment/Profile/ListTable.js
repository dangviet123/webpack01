...
function ListTable(props) {
    ...
    const renderLitsMenu = () => {
        ...
        xhtml = datas.map((data,index) => {
            const idCheck = `popover-${data.idProfile}`;
            return (
                <TableRow hover key={index}>
                    ...
                    <TableCell align="center">
                        <Tooltip title={langs.see_more} placement="right"   arrow>
                            <IconButton 
                                onClick={(event) => handleShowDetailClick(event, idCheck,data.idProfile)}
                                id={idCheck}
                            >
                    ...
                    <TableCell align="center">{data.Recruit}</TableCell>
                    <TableCell align="center">{data.Email}</TableCell>
                    <TableCell align="center">{data.Phone}</TableCell>
                    <TableCell align="center"><Moment format="DD-MM-YYYY">{data.DateAdd}</Moment></TableCell>
                    <TableCell align="center">
                        <Tooltip title={data.Check ? langs.active_process : langs.inactive_process} placement="left"  arrow>
                            {checkProcessOneSave(arrPermission) === true ? <IconButton 
                            color={data.Check ? `primary` : `secondary`}  
                            component="span"
                            onClick={() => checkProcessOne(data.idProfile,data.Check)}
                            >
                                {data.Check ? <CheckIcon />: <CloseIcon />} 
            ...
            <Table aria-label="simple table" >
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="center">{langs.full_name}</TableCell>
                        ...
                        <TableCell align="center" width={120}>{langs.date_send}</TableCell>
