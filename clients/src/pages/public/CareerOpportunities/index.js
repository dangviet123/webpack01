...
function CareerOpportunities() {
    ...
    useEffect(() => {
       document.title = `${langs.career_opportunities} | ${langs.name_company}`;
       window.scrollTo(0, 0);
    },[]);
    ...
    const list_prc = [
        {Name: langs.career_opportunities,Slug: '/career-opportunities',Active: 1}
    ];
    const {datas} = useSelector(state => state.Header);
    const NewData = datas.filter(d => d.Slug === '/career-opportunities');

    const handleShowVideo = () => {
        dispatch(showModalDialogs(true,'md',langs.career_opportunities,<ShowVideo Post={Post}  />));
    }
    ...
    return (
        <main>
            ...
            <section className="sukiennoibat-1">
