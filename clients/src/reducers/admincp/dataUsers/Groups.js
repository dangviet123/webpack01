...
const Groups = (state = initialState, action) => {
    switch (action.type) {
        ...
        case Types.FETCH_GROUPS_CHECKED_ONE:
            const {index} = action.payload;
            state.datas[index].checked= !state.datas[index].checked;
            return {...state};
        case Types.FETCH_GROUPS_CHECKED_ALL:
            const {checkAll} = action.payload;
            state.datas.forEach((el,index) => {state.datas[index].checked = checkAll;});
            return {...state};
        case Types.FETCH_GROUPS_ACTIVE_ONE: // active one
            const {idGroups,active} = action.payload;
            state.datas.forEach((el,index) => {
                if (state.datas[index].idGroups === idGroups)   
                    state.datas[index].Active = active ? 1: 0;
                    state.datas[index].checked = false;
            });
            return {...state};
        case Types.FETCH_GROUPS_ACTIVE_ALL_SUCCESS: // active all
            const { list_id } = action.payload;
            state.datas.forEach((el,index) => {
                if ( list_id.includes(state.datas[index].idGroups))
