...
const Vacancies = (state = initialState, action) => {
    switch (action.type) {
        ...
        case Types.FETCH_VACANCIES_CHECKED_ONE:{
            const {index} = action.payload;
            state.datas[index].checked= !state.datas[index].checked;
            return {...state};
        }
            
        case Types.FETCH_VACANCIES_CHECKED_ALL:{
            const {checkAll} = action.payload;
            state.datas.forEach((el,index) => {state.datas[index].checked = checkAll;});
            return {...state};
        }
            
        case Types.FETCH_VACANCIES_ACTIVE_ONE:{
            const {idVacancies,active} = action.payload;
            state.datas.forEach((el,index) => {
                if (state.datas[index].idVacancies === idVacancies)   
        ...
        case Types.FETCH_VACANCIES_ACTIVE_ALL_SUCCESS:{
            const { list_id } = action.payload;
            state.datas.forEach((el,index) => {
                if ( list_id.includes(state.datas[index].idVacancies))
